...
import { X, Tag as TagIcon, Save, Send, AlertCircle } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        if (data.success) {
          // Remove data we don't want to modify from the API response
          const { _id, createdAt, updatedAt, author, slug, ...postData } = data.data;
          setPost(postData);
        } else {
          setLoadError('Failed to load post data');
        }
      } catch (error) {
